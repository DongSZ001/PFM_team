...
│   │  /auth/*   → auth.js (注册/登录)          │      │
│   │  /chat/*   → auth.js (会话/消息)          │      │
│   │  /webui/*  → bridge:8765 (nanobot 桥)    │      │
│   │  其他      → nanobot/web/dist (fallback)  │      │
│   └──────────────────────────────────────────┘      │
│   ┌──────────────────────────────────────────┐      │
│   │ WebSocket 代理                             │      │
...
│   ├── styles.css                     主题、布局、组件样式
│   └── app.js                         前端逻辑 (auth/chat/gateway)
│
└── pf_assistant/                      后端服务 (~20MB)
    │
    ├── serve.js                       主入口 (HTTP + WebSocket 代理)
    ├── auth.js                        认证/会话 API
...
创建 systemd service `/etc/systemd/system/pfm2-webui.service`：

```ini
[Unit]
Description=PFM2 WebUI Server
After=network.target

[Service]
Type=simple
User=admin
WorkingDirectory=/home/admin/.openclaw/workspace/pf-assistant-webui/pf_assistant
ExecStart=/usr/bin/node serve.js
Restart=always
RestartSec=10
StandardOutput=append:/home/admin/.openclaw/workspace/pf-assistant-webui/pf_assistant/logs/app.log
StandardError=append:/home/admin/.openclaw/workspace/pf-assistant-webui/pf_assistant/logs/app.log
...
```bash
sudo systemctl enable pfm2-webui
sudo systemctl start pfm2-webui
```
