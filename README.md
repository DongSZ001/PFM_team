...
### 铁电相场计算
- **对话触发**：在聊天框输入“铁电畴结构计算”“极化分布”“相场模拟”等请求，前端会进入铁电计算参数向导。
- **计算流程**：后端为每次任务创建独立 job 目录，复制 Fortran 模板，执行 `make clean`、`make`、`./main.exe`。
- **结果输出**：读取 `Polar.*******.dat`，生成极化分布 PNG，并在聊天消息中以图片卡片展示。
- **默认模型**：第一阶段默认使用 PMN-PT 参数、`pz` 分量和 `x-z` 截面，可通过对话补充网格、步数和输出间隔。
